...
class ScalaLanguageServer extends LanguageServer
  ...
  private[this] var myDrivers: mutable.Map[IDEConfig, InteractiveDriver] = _

  def drivers: mutable.Map[IDEConfig, InteractiveDriver] = thisServer.synchronized {
    if (myDrivers == null) {
      assert(rootUri != null, "`drivers` cannot be called before `initialize`")
      val configFile = new File(new URI(rootUri + '/' + IDE_CONFIG_FILE))
      val configs: List[IDEConfig] = (new ObjectMapper).readValue(configFile, classOf[Array[IDEConfig]]).toList

      val defaultFlags = List(/*"-Yplain-printer","-Yprintpos"*/)

      myDrivers = new mutable.HashMap
      for (config <- configs) {
        val classpathFlags = List("-classpath", (config.target +: config.depCp).mkString(":"))
        val settings = defaultFlags ++ config.scalacArgs.toList ++ classpathFlags
  ...
  def driverFor(uri: URI): InteractiveDriver = {
    val matchingConfig =
      drivers.keys.find(config => config.sources.exists(sourceDir =>
        uri.getRawPath.startsWith(sourceDir.getAbsolutePath.toString)))
    matchingConfig match {
      case Some(config) =>
        drivers(config)
      case None =>
        println(s"No configuration contains $uri as a source file, an arbitrary driver will be used.")
        drivers(drivers.keys.head)
    }
  }
  ...
  override def definition(params: TextDocumentPositionParams) = computeAsync { cancelToken =>
    ...
    val pos = sourcePosition(driver, uri, params.getPosition)
    val sym = Interactive.enclosingSymbol(driver.openedTrees(uri), pos)
  ...
  override def documentHighlight(params: TextDocumentPositionParams) = computeAsync { cancelToken =>
    ...
    val sym = Interactive.enclosingSymbol(uriTrees, pos)
