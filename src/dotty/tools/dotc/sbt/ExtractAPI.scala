...
private class ExtractAPICollector(implicit val ctx: Context) extends ThunkHolder {
  ...
  private def computeClass(sym: ClassSymbol): api.ClassLike = {
    ...
    new api.ClassLike(
      defType, strictlzy(selfType), strictlzy(structure), Constants.emptyStringArray,
  ...
  def apiClassStructure(csym: ClassSymbol): api.Structure = {
    ...
    new api.Structure(strictlzy(apiBases.toArray), strictlzy(apiDecls.toArray), apiInherited)
  }
  ...
  private def computeType(tp: Type): api.Type = {
    ...
    tp2 match {
      ...
      case rt: RefinedType =>
        ...
        new api.Structure(strictlzy(Array(parent)), strictlzy(decl), strictlzy(Array()))
      case tp: AndOrType =>
        ...
        // be symmetric.
        val s = new api.Structure(strictlzy(parents.toArray), strictlzy(Array()), strictlzy(Array()))
        if (tp.isAnd)
          s
        else
