    ...
    */
    ...
    private def completeFrom(completer: LazyType)(implicit ctx: Context): Unit = {
      ...
      if ((curThread ne null)) {
        val noCycle = ctx.addToWaitList(curThread, this, s"$this while compiling ${if (ctx.compilationUnit ne null) ctx.compilationUnit.source else ""}")
        if (!noCycle) {
          println("yesCycle")
          throw CyclicReference(this)
        }
        ...
        this.synchronized {
          try {
            if (this.toString == "class ClassNotFoundException")
              println("thread " + Thread.currentThread() + s" is initializing $this")
...
*/
    ...
    private def computeBases(implicit ctx: Context): (List[ClassSymbol], BitSet) = synchronized {
