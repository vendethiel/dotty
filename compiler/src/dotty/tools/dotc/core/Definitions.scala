      ...
      def complete(denot: SymDenotation)(implicit ctx: Context): Unit = {
        ...
        val argParams =
          for (i <- List.range(0, arity)) yield
            enterTypeParam(cls, (name.toString + "$T" + i).toTypeName, Contravariant, decls, phantomicity.tParamBounds(i)).typeRef
        val resParam = enterTypeParam(cls, (name.toString + "$R").toTypeName, Covariant, decls, phantomicity.tParamBounds(arity)).typeRef
